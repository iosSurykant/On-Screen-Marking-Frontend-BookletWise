...
const ReassignModal = ({
  ...
  const [reassignType, setReassignType] = useState("pending");

  /* ------------------------------------------------
     FETCH CURRENT USER TASK STATUS
  ------------------------------------------------- */
  useEffect(() => {
    if (!showPendingModal || !currentTask?.userId?._id) return;
  ...
  useEffect(() => {
    ...
    if (matchedTask) {
      ...
      setTransferCount(0);
      setMaxTransferCount(pending);
    }
  }, [userTaskStatus, currentTask]);

  const handleReassignType = (type) => {
    setReassignType(type);

    if (type === "completed") {
      setMaxTransferCount(completedCount);
    } else {
      setMaxTransferCount(pendingCount);
    }
  ...
  if (!showPendingModal || !currentTask) return null;
    ...
    <div className="bg-black fixed inset-0 z-50 flex items-center justify-center bg-opacity-50 backdrop-blur-md">
      <div className="w-[460px] rounded-xl bg-white shadow-lg dark:bg-navy-700 dark:text-white">
        ...
        <div className="flex justify-between border-b px-4 py-3">
          <h2 className="text-xl font-bold">Reassign Pending Task</h2>
          <span
        ...
        <div className="px-4 py-2">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => handleReassignType("pending")}
              className={`flex-1 rounded-lg border py-2 text-sm font-medium ${
                reassignType === "pending"
            ...
            <button
              type="button"
              onClick={() => handleReassignType("completed")}
              className={`flex-1 rounded-lg border py-2 text-sm font-medium ${
        ...
        <div className="px-4 py-2">
          <label className="mb-1 block text-sm font-medium">
            Transfer {reassignType} Booklets
          </label>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Available Count: {maxTransferCount}
          </p>
        </div>
